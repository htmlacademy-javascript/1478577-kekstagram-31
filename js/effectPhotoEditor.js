...
const updateSliderOptions = ({min, max, start, step, filterName, unit}) =>{
  ...
  slider.noUiSlider.on('update', (values, handle) =>{
    ...
    previewPictures.style.filter = `${filterName}(${effectLevelValue.value}${unit})`;
  });
};
...
const changePhotoEffect = (evt) => {
  ...
  switch (effect){
    case 'none':
      previewPictures.style.filter = 'none';
      break;
    case 'chrome':
